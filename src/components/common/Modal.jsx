...
export function Modal({ isOpen, onClose, title, children, maxWidth = 'max-w-2xl' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-6 overflow-y-auto pb-safe">
      ...
      <div
        className={cn(
          'relative w-full bg-slate-900 border-t sm:border border-slate-800 rounded-t-3xl sm:rounded-2xl shadow-2xl z-10 my-0 sm:my-8 overflow-hidden transition-all transform animate-in slide-in-from-bottom duration-200 sm:animate-scale-up',
          maxWidth
        )}
      >
