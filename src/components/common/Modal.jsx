...
export function Modal({ isOpen, onClose, title, children, maxWidth = 'max-w-2xl' }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-6 overflow-y-auto pb-safe">
      ...
      <div
        className={cn(
          'relative w-full bg-white border-t sm:border border-slate-200 rounded-t-3xl sm:rounded-2xl shadow-2xl z-10 my-0 sm:my-8 overflow-hidden transition-all transform animate-in slide-in-from-bottom duration-200 sm:animate-scale-up',
          maxWidth
        )}
      >
        ...
        {/* Content */}
        <div className="px-4 sm:px-6 py-4 sm:py-5 max-h-[80vh] overflow-y-auto overscroll-contain text-slate-800">
          {children}
        </div>
      </div>
    </div>
