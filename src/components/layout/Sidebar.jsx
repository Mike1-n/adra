...
export function Sidebar({ currentTab, onSelectTab, onSwitchToAdminWeb, isMobile = false, onCloseMobile }) {
  ...
  return (
    <aside className="w-64 bg-slate-950/95 border-r border-slate-800/80 flex flex-col h-full select-none">
      {/* Brand Header */}
      <div className="h-16 flex items-center gap-3 px-5 border-b border-slate-800 bg-white">
        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-emerald-500 to-emerald-700 flex items-center justify-center text-white font-black text-base shadow-md shadow-emerald-500/20 border border-emerald-400/30">
          A
        </div>
        <div className="flex flex-col">
          <span className="font-extrabold text-base tracking-tight text-slate-100 flex items-center gap-1.5">
            ADRA <span className="text-emerald-600 text-xs font-semibold px-1.5 py-0.5 rounded bg-emerald-50 border border-emerald-500/30">DMS</span>
          </span>
          <span className="text-[10px] text-slate-400 uppercase tracking-wider">Field Application</span>
        </div>
      </div>

      {/* Role Switcher for Quick Academic Defense */}
      <div className="p-3 mx-3 my-2.5 rounded-xl bg-slate-850 border border-slate-800">
        <div className="flex items-center justify-between text-[11px] text-slate-500 mb-1.5">
          <span className="flex items-center gap-1 font-medium text-emerald-600">
            <Sparkles className="w-3 h-3" /> Quick Role Switcher
          </span>
          <span className="text-[9px] text-slate-400 uppercase">Viva / Demo</span>
        </div>
        <select
          value={currentUser?.role || 'Administrator'}
          onChange={(e) => quickSwitchRole(e.target.value)}
          className="w-full bg-white border border-slate-800 rounded-lg px-2.5 py-1.5 text-xs text-slate-100 focus:outline-none focus:border-emerald-500 cursor-pointer font-medium"
        >
      ...
      {currentUser?.role === 'Administrator' && onSwitchToAdminWeb && (
        <div className="mx-3 mb-2 p-3 rounded-xl bg-emerald-50/60 border border-emerald-500/30 shadow-sm">
          <div className="flex items-center justify-between text-[11px] mb-1">
            <span className="font-bold text-emerald-700 flex items-center gap-1.5">
              <ShieldCheck className="w-3.5 h-3.5" /> Admin Web App
            </span>
            {pendingApprovals > 0 && (
              <span className="px-1.5 py-0.2 text-[9px] font-bold rounded-full bg-amber-100 text-amber-800 border border-amber-300 animate-pulse">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs shadow-sm transition"
          >
          ...
          return (
            <button
              ...
              className={cn(
                ...
                isActive
                  ? 'bg-emerald-50 text-emerald-700 border border-emerald-500/30 shadow-sm font-semibold'
                  : 'text-slate-500 hover:text-slate-100 hover:bg-slate-850 border border-transparent'
              )}
            >
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                {item.id === 'users' && pendingApprovals > 0 && (
                  <span className="px-1.5 py-0.5 text-[10px] font-bold rounded-full bg-amber-100 text-amber-800 border border-amber-300 animate-pulse">
                    {pendingApprovals}
                  </span>
                )}
                {isActive && (
      ...
      {/* User Profile Footer */}
      <div className="p-3 border-t border-slate-800 bg-white">
        <div className="flex items-center gap-3 p-2 rounded-xl bg-slate-850 border border-slate-800">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] text-emerald-600 truncate flex items-center gap-1 font-medium">
