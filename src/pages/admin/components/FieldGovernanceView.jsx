...
export function FieldGovernanceView({ initialTab = 'programmes' }) {
  ...
  const handleOpenCreateProg = () => {
    ...
    setProgForm({
      ...
      start_date: new Date().toISOString().split('T')[0],
      end_date: new Date(Date.now() + 365 * 24 * 3600000).toISOString().split('T')[0],
      location: locations[0]?.name || 'Turkana County Office',
      status: 'Active',
      manager_name: managers[0]?.full_name || 'Grace Ochieng'
    });
  ...
  const handleOpenEditProg = (p) => {
    ...
    setProgForm({
      ...
      manager_name: p.manager_name || (managers[0]?.full_name || 'Program Manager')
    });
            ...
            {programmes.map((p) => (
              <Card key={p.id} className="adra-card-hover flex flex-col justify-between">
                <div>
                  ...
                  <div className="space-y-1.5 mt-3 pt-3 border-t border-slate-800 text-xs text-slate-300">
                    <div className="flex items-center justify-between">
                      <span className="text-slate-500 text-[11px] flex items-center gap-1">
                        <User className="w-3 h-3 text-emerald-400" /> Responsible Manager:
                      </span>
                      <span className="font-medium text-white">{p.manager_name || 'Program Manager (Assigned)'}</span>
                    </div>

                    <div className="flex items-center justify-between">
                      <span className="text-slate-500 text-[11px] flex items-center gap-1">
                        <MapPin className="w-3 h-3 text-slate-500" /> Operational Zone:
                      </span>
                      <span className="truncate max-w-[170px]">{p.location}</span>
                    </div>

                    <div className="flex items-center justify-between">
                      <span className="text-slate-500 text-[11px] flex items-center gap-1">
                        <DollarSign className="w-3 h-3 text-amber-400" /> Budget Allocation:
                      </span>
                      <span className="font-semibold text-emerald-400">{formatCurrency(p.budget)}</span>
                    </div>
                  </div>
                </div>
                ...
                <tbody className="divide-y divide-slate-800/60 text-slate-200">
                  {locations.map((loc) => (
                    <tr key={loc.id} className="hover:bg-slate-900/50 transition">
                      ...
                      <td className="py-3.5 px-4 text-slate-300">{loc.county} / {loc.district}</td>
                      <td className="py-3.5 px-4 text-slate-400">{loc.community}</td>
                      <td className="py-3.5 px-4 font-mono text-[11px] text-slate-400">{loc.contact || 'N/A'}</td>
      ...
      >
        <form onSubmit={isEditProgOpen ? handleSaveEditProg : handleCreateProg} className="space-y-4 text-xs">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block font-semibold text-slate-300 mb-1">Programme Code</label>
              <input
                type="text"
                required
            ...
            <div>
              <label className="block font-semibold text-slate-300 mb-1">Programme Status</label>
              <select
          ...
          <div>
            <label className="block font-semibold text-slate-300 mb-1">Programme Name</label>
            <input
              type="text"
              required
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block font-semibold text-slate-300 mb-1">Assigned Program Manager</label>
              <select
            ...
            <div>
              <label className="block font-semibold text-slate-300 mb-1">Budget Allocation ($ USD)</label>
              <input
                type="number"
                required
          ...
          <div>
            <label className="block font-semibold text-slate-300 mb-1">Operational Location</label>
            <input
              type="text"
              required
          ...
          <div>
            <label className="block font-semibold text-slate-300 mb-1">Programme Objectives / Scope</label>
            <textarea
              rows={3}
      ...
      >
        <form onSubmit={handleCreateLoc} className="space-y-4 text-xs">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="block font-semibold text-slate-300 mb-1">Facility Type</label>
              <select
                value={locForm.type}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="block font-semibold text-slate-300 mb-1">County / District</label>
              <input
                type="text"
                required
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block font-semibold text-slate-300 mb-1">Community Camp / Sector</label>
              <input
                type="text"
